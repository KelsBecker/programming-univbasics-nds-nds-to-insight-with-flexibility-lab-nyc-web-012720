...
def gross_per_studio(collection)
    ...
    while counter < collection.length do 
      ...
      if !gross[movie[:studio]] 
        gross[movie[:studio]] = movie[:worldwide_gross]
      else 
        gross[movie[:studio]] += movie[:worldwide_gross]
      end
      counter += 1 
...
def movies_with_directors_set(source)
  a_o_a_directors_movies = []
  counter = 0 
  
  while counter < source.length do 
    director_info_hash = source[counter]
    director_name = director_info_hash[:name]
    director_movies = director_info_hash[:movies]
    a_o_a_directors_movies << movies_with_director_key(director_name, director_movies)
